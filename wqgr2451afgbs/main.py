...
async def start_topup(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введите сумму пополнения в рублях (от 25):")
...
async def process_topup_amount(message: types.Message, state: FSMContext):
    try:
        amount = float(message.text)
        if amount < 25 or amount > 1000000:
            await message.answer("❌ Сумма должна быть от 25 RUB. Попробуйте снова.")
            return
